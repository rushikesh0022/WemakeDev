...
export async function POST(_request: Request, { params }: { params: Promise<{ splitId: string; settlementId: string }> }) {
  ...
  if (!user) return NextResponse.json({ error: "Sign in to review this payment." }, { status: 401 });
  const { splitId, settlementId } = await params;
  try {
    const split = await reviewSettlement(user.id, splitId, settlementId, "reject");
    return split ? NextResponse.json({ split }) : NextResponse.json({ error: "Settlement was not found." }, { status: 404 });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Could not reject payment." }, { status: 400 });
  }
}
