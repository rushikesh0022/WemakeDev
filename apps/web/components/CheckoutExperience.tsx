...
export function CheckoutExperience({ user, allowPaymentSimulation = false }: { user: PublicUser; allowPaymentSimulation?: boolean }) {
  ...
  if (orderId) return <section className="order-success order-success--nesto"><div className="order-success__visual"><i /><img src="/nesto/motion/order-journey-v2.webp" alt="Zaply scooter beginning a delivery" /><span className="order-success__pulse" /></div><div className="order-success__badge"><Check /></div><span>ORDER CONFIRMED</span><h1>Your delivery is in motion.</h1><p>Payment for {orderId} is verified. We are checking and packing every item now.</p><div className="order-success__steps"><span className="active"><Check /> Confirmed</span><i /><span><PackageCheck /> Packing</span><i /><span><Clock3 /> On the way</span></div><Link className="primary-button" href={`/orders/${orderId}`}>Track live order</Link><Link className="text-link" href="/">Continue shopping</Link></section>;
  ...
  return (
    <div className="checkout-page">{placing && <div className="checkout-motion" role="status" aria-live="polite"><div><img src="/nesto/motion/intro-bag-v2.webp" alt="" /><span>Securing your basket</span><strong>Confirming payment with the provider…</strong><i /></div></div>}<header className="checkout-head"><span>SECURE CHECKOUT</span><h1>Review and place your order</h1><p>Confirm the address and pay securely. Zaply marks an order paid only after server verification.</p><div className="checkout-steps"><span className="done"><Check /> Cart</span><i /><span className="active">2</span><b>Payment</b><i /><span>3</span><b>Confirmation</b></div></header>
      <div className="checkout-layout"><section className="checkout-details">
        <article><MapPin /><div><span>DELIVERY DESTINATION</span><h2>{user.address?.label ?? "Add an address"}</h2><p>{user.address ? `${user.address.line1}, ${user.address.city} ${user.address.pincode}` : "Save an address in your account before delivery."}</p></div><Link href="/account?view=addresses&next=/checkout">{user.address ? "Change" : "Configure"}</Link></article>
