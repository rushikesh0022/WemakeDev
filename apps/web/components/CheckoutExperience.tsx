...
export function CheckoutExperience({ user, allowPaymentSimulation = false }: { user: PublicUser; allowPaymentSimulation?: boolean }) {
  ...
  if (orderId) return <section className="order-success"><div><Check /></div><span>PAYMENT CONFIRMED</span><h1>Your basket is packed.</h1><p>Order {orderId} is paid and ready to split with friends.</p><Link className="primary-button" href="/account?view=orders">Split or view order</Link></section>;
  ...
  return (
    <div className="checkout-page"><header><span>CHECKOUT</span><h1>One last review</h1><p>The local payment adapter mirrors the states required for Amazon Pay sandbox integration.</p></header>
      <div className="checkout-layout"><section className="checkout-details">
        ...
        <article><ShieldCheck /><div><span>PAYMENT</span><h2>Amazon Pay or UPI</h2><p>Zaply confirms the payment server-side before marking the order paid.</p></div></article>
