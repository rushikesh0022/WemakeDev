...
export function CheckoutExperience({ user, allowPaymentSimulation = false }: { user: PublicUser; allowPaymentSimulation?: boolean }) {
  ...
  if (orderId) return <section className="order-success order-success--nesto"><div className="order-success__visual"><i /><img src="/nesto/motion/order-journey-v2.webp" alt="Nesto scooter beginning a delivery" /><span className="order-success__pulse" /></div><div className="order-success__badge"><Check /></div><span>ORDER CONFIRMED</span><h1>Your delivery is in motion.</h1><p>Payment for {orderId} is verified. We are checking and packing every item now.</p><div className="order-success__steps"><span className="active"><Check /> Confirmed</span><i /><span><PackageCheck /> Packing</span><i /><span><Clock3 /> On the way</span></div><Link className="primary-button" href={`/orders/${orderId}`}>Track live order</Link><Link className="text-link" href="/">Continue shopping</Link></section>;
